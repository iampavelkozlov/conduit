...
func initializeApplication(ctx context.Context, configPath string) (*application, func(), error) {
	...
	postgresRepository := provideArticleRepository(pool, querier, v)
	...
	articleService := provideArticleService(postgresRepository, userService, tagService, articletagService, favoriteService, followService)
	authService := provideAuthService(querier, configConfig)
